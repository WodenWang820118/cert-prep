...
test('installs a valid release and reuses identical staging', async () => {
  ...
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'cert-prep-capture-runtime-test-'));
...
test('fails closed when an existing pinned staging has different bytes', async () => {
  ...
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'cert-prep-capture-runtime-test-'));
  temporaryRoots.push(workspaceRoot);
  const installed = await installCaptureRuntime({ workspaceRoot, baseUrl: mirror });
...
test('rejects executable, schema, manifest, checksum, and platform drift', async () => {
  const root = await createFixture();
  const mutations: ReadonlyArray<readonly [string, (target: string) => Promise<void>]> = [
    ['executable', (target) => writeFile(join(target, CAPTURE_RUNTIME_FILE), 'tampered')],
    ['schema', (target) => writeFile(join(target, CAPTURE_DOCUMENT_SCHEMA_FILE), 'tampered')],
    [
      'executable bytes',
      async (target) => {
        const manifest = await readManifest(target);
        manifest.bytes = Number(manifest.bytes) + 1;
        await writeFile(join(target, 'capture-runtime-manifest.json'), JSON.stringify(manifest));
      },
    ],
    [
      'executable digest',
    ...
    ['unexpected artifact', (target) => writeFile(join(target, 'unexpected.txt'), 'rejected')],
  ];
...
test('rejects HTTP redirects instead of following them', async () => {
  ...
  if (!address || typeof address === 'string') throw new Error('Redirect server did not expose a port.');
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'cert-prep-capture-runtime-redirect-'));
  temporaryRoots.push(workspaceRoot);
...
async function createFixture(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'cert-prep-capture-runtime-release-'));
  ...
  await writeFile(join(root, 'capture-runtime-manifest.json'), JSON.stringify(manifest));
...
async function readManifest(root: string): Promise<Record<string, unknown>> {
  return JSON.parse(await readFile(join(root, 'capture-runtime-manifest.json'), 'utf8')) as Record<string, unknown>;
}

async function cloneFixture(source: string): Promise<string> {
  const target = await mkdtemp(join(tmpdir(), 'cert-prep-capture-runtime-release-copy-'));
...
async function startMirror(root: string): Promise<string> {
  const server = createServer((request, response) => {
    ...
    if (request.method !== 'GET' || !name || !CAPTURE_RUNTIME_RELEASE_ASSETS.includes(name)) {

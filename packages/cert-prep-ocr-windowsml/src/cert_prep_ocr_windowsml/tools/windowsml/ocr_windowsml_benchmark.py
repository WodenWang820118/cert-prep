...
from cert_prep_ocr_windowsml.contracts import OCRPageResult
from cert_prep_ocr_windowsml.paths import DEFAULT_OUTPUT_DIR, REPO_ROOT
from cert_prep_ocr_windowsml.runtime import WindowsMLOCRRunner
from cert_prep_ocr_windowsml.tools.windowsml.ocr_windowsml_inference_smoke import (
    build_report as build_inference_report,
)
from cert_prep_ocr_windowsml.tools.windowsml.ocr_windowsml_probe import DEFAULT_MODEL_DIR
from cert_prep_ocr_windowsml.tools.windowsml.ocr_windowsml_smoke import WINDOWSML_DEVICE_LABEL
...
DEFAULT_PAGE_3_ANCHORS = ["問題2", "ネギ", "講師", "資格"]
...
def benchmark_pdf_page(
    *,
    ...
    cold = provider.extract_page_text(image_png, page_number)
    warm = provider.extract_page_text(image_png, page_number)
    text = warm.text
    ...
    return asdict(
        OCRBenchmarkResult(
            ...
            anchors_present={anchor: anchor in text for anchor in anchors},
        )
    )

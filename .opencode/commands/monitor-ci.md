...
3. **ci-poll-decide.mts (deterministic script)**: takes ci_information result + state, returns action + status message
4. **ci-state-update.mts (deterministic script)**: manages budget gates, post-action state transitions, and cycle classification
...
- **Gate check**: Run `ci-state-update.mts gate` before local fix attempts — if budget exhausted, print message and exit
...
```bash
node <skill_dir>/scripts/ci-poll-decide.mts '<subagent_result_json>' <poll_count> <verbosity> \
  [--wait-mode] \
...
```bash
node <skill_dir>/scripts/ci-state-update.mts post-action \
  --action <type> \
...
```bash
node <skill_dir>/scripts/ci-state-update.mts cycle-check \
  --code <code> \
  [--agent-triggered] \
...
- `no_progress_count`, circuit breaker (5 polls), and backoff reset are handled by ci-poll-decide.mts (progress = any change in cipeStatus, selfHealingStatus, verificationStatus, or failureClassification)
- `env_rerun_count` reset on non-environment status is handled by ci-state-update.mts cycle-check

...
- Suggested implementation slices: package scaffold, runtime module move, backend target updates to package modules, focused verification.
- Behavior-preservation checks: existing WindowsML unit tests and backend lint; project graph includes package.
- Review/checkpoint needs: keep backend target names stable so existing root scripts and packaging lanes remain callable, while retiring old compatibility import paths.
- Decision: normal implementation with a narrow package extraction; do not invoke a broad refactor worker.

## Chosen Shape

Create `packages/cert-prep-ocr-windowsml` as a Python package with its own `pyproject.toml`, `project.json`, source tree, and tests. The backend app depends on it through a local editable/path dependency. Backend `ocr-windowsml-*` Nx target names stay stable, but their commands execute `cert_prep_ocr_windowsml.tools.windowsml.*` modules directly; backend compatibility shims are intentionally retired.

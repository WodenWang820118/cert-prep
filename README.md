...
`capture-runtime@0.4.0` is installed as a Windows x64 release sidecar, not as
an npm or Python dependency. By default, the installer uses the canonical
`https://github.com/gx-capture/capture-workbench/releases/download/v0.4.0`
...
under `tmp/cert-prep/capture-runtime/0.4.0`. Desktop preparation consumes that
...
v0.4.0 is the engine-bearing release contract. Cert Prep uses
...
`@gx-capture/capture-workbench-ui@0.4.0` from GitHub Packages. GitHub Actions configures
...
the v0.4.0 sidecar, the route accepts embedded-text PDFs and exposes image and
...
published `@gx-capture/capture-workbench-ui@0.4.0` Web Component. The `/build`
...
registry-installed `@gx-capture/capture-workbench-ui@0.4.0` package and a cert-prep
`CaptureClient` backed by the review-gated capture API. Capture Runtime and its
token remain backend-only. With the v0.4.0 engine-bearing sidecar, the route

...
test('dxgi adapter reader accepts PowerShell UTF-8 BOM JSON', () => {
  const dir = mkdtempSync(join(tmpdir(), 'exam-prep-dxgi-'));
  try {
...
test('resource summary finalizer preserves closeout evidence and target process GPU usage', () => {
  const dir = mkdtempSync(join(tmpdir(), 'exam-prep-resource-summary-'));
  try {
    writeFileSync(
...
"2026-06-21T00:00:00Z","windows_process","Win32_Process","42","exam-prep-ocr-runtime.exe","working_set_bytes","1024","bytes"
...
`);
  ...
  assert.equal(summary.target_process_gpu_usage[0].name, 'exam-prep-ocr-runtime.exe');
  assert.equal(summary.named_target_process_gpu_usage.length, 1);
  assert.equal(
    summary.named_target_process_gpu_usage[0].name,
    'exam-prep-ocr-runtime.exe',
...
test('adapter-aware summary exposes DirectML OCR and reasoning GPU routing gates', () => {
  const windowsSummary = summarizeWindowsResourceCsv(`timestamp,source,path,pid,name,metric,value,unit
"2026-06-21T00:00:00Z","windows_process","Win32_Process","42","exam-prep-ocr-directml-runtime.exe","working_set_bytes","1024","bytes"

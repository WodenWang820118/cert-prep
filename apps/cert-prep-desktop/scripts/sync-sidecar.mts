...
const SIDECAR_PATTERN = /^cert-prep-backend-.+(?:\.exe)?$/;
const scriptDir = dirname(fileURLToPath(import.meta.url));
const workspaceRoot = resolve(scriptDir, '../../..');
const backendDist = join(workspaceRoot, 'apps/cert-prep-backend/dist');
const desktopBinaries = join(
  workspaceRoot,
  'apps/cert-prep-desktop/src-tauri/binaries'
);
...
const sourceName = isWindows ? 'cert-prep-backend.exe' : 'cert-prep-backend';
const targetExtension = targetTriple.includes('windows') ? '.exe' : '';
const sourcePath = join(backendDist, sourceName);
const targetPath = join(
  desktopBinaries,
  `cert-prep-backend-${targetTriple}${targetExtension}`
);

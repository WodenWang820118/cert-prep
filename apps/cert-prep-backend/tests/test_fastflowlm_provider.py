...
def test_fastflowlm_onboarding_probes_runtime_inventory_check_and_generation(
    monkeypatch,
) -> None:
    ...
    executable = Path("C:/Program Files/flm/flm.exe")
    ...
    def run_cli_json(_executable: Path, args: list[str]) -> dict:
        cli_calls.append(tuple(args))
        if args[0] == "validate":
            return {"ready": True, "amd_device_found": True, "npu_driver_ok": True}
        return {"models": [{"model": "qwen3.5:4b", "installed": True}]}

    def run_cli(_executable: Path, args: list[str]) -> str:
        cli_calls.append(tuple(args))
        return "model check passed"

    monkeypatch.setattr(provider, "_run_cli_json", run_cli_json)
    monkeypatch.setattr(provider, "_run_cli", run_cli)

    provider.prepare_model_onboarding(lambda item: progress.append(item.status))
    provider.verify_model_onboarding(lambda item: progress.append(item.status))
    ...
    assert any("tiny completion passed with qwen3.5:4b" in item for item in progress)

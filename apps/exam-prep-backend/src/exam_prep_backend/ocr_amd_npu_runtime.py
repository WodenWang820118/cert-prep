...
def main() -> None:
    ...
    parser.add_argument("--provider", choices=["amd_npu"], default="amd_npu")
...
def _provider_from_args(args: argparse.Namespace) -> AmdNpuRuntimeOCRProvider:
    return AmdNpuRuntimeOCRProvider(
        model_dir=args.model_dir or _default_model_dir(),
        directml_device_id=args.directml_device_id,
        npu_device_id=args.amd_npu_device_id,
        npu_policy=args.amd_npu_policy,
...
def _run_worker(provider: AmdNpuRuntimeOCRProvider) -> None:
...
def _worker_response(provider: AmdNpuRuntimeOCRProvider, line: str) -> dict[str, Any]:
